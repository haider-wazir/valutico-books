...
const { mutate: deleteInvoice, onDone } = useMutation(
  gql`
...
onDone((res) => {
  const id = res.data.deleteBook.id;
  this.result.books.splice(
    this.books.indexOf((book) => book.id === id),
    1,
  );
});
</script>
      ...
      <div class="form-box pt-25">
        <div class="form-bg py-30 px-50 mw-none">
          <div class="flex">
            ...
            <router-link :to="{ path: '/new' }" class="ml-auto"
          ...
          <table class="navbar-nav">
            ...
            <tbody>
              <tr v-for="book in result?.books" :key="book.id" class="nav-item">
                <td>{{ book.id }}</td>
                <td>{{ book.title }}</td>
                <td>{{ book.author }}</td>
